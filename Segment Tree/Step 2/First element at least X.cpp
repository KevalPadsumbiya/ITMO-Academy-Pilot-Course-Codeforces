...
int query(int index,int start,int end,int value,int segtree[])
{
	if(segtree[1]<value)
		return -1;
	if(start==end)
		return start;
	int mid=(start+end)/2;
	if(segtree[index<<1]>=value)
		query(index<<1,start,mid,value,segtree);
	else
		query(index<<1|1,mid+1,end,value,segtree);	
}
